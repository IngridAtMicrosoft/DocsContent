...
**Note**: Currently, you can't use the Azure portal to manage v3 resources. Use the [REST API](/rest/api/media/account-filters), [Azure CLI](/cli/azure/ams?view=azure-cli-latest), or one of the supported [SDKs](/azure/media-services/latest/all-sdks).
...
- In Media Services v3, [live events](/azure/media-services/latest/live-event-outputs-concept) are responsible for ingesting and processing the live video feeds. Once the live event is created, you can get ingest URLs that you'll provide to the live on-premises encoder. The live encoder uses these URLs to input a live stream. See the recommended on-premises live encoders above. You can either use non-vanity URLs or vanity URLs.
...
* [Live Events](/azure/media-services/latest/live-event-outputs-concept)
